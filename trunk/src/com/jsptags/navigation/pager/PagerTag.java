...
//	private String maxPageItemsKey = null;
	
	
	private String itemsKeyName= null;
	private String itemskeyProperty=null;
	
	private String maxPageItemsKeyName=null;
	private String maxPageItemsKeyProperty=null;
	/*
	 * Tag Variables
	 */
	...
	final void addParamScope(String name, String value,String scope,String property) {
...
//		} else {
			...
			values = pageContext.getRequest().getParameterValues(name);
			
			if(null ==values){
				
				values = getScopeValue(name,scope,property);
			}

			if (values != null) {
				
//				name = java.net.URLEncoder.encode(name);
				property = java.net.URLEncoder.encode(property);
				for (int i = 0, l = values.length; i < l; i++) {
					
					if(StringCommonUtil.isEmpty(values[i]))
						continue;
...
//		}
	...
	final String[] getScopeValue(String name,String scope,String property){
		String[] values = null;
		
		if(PAGE.equalsIgnoreCase(scope)){
			
		}else if(REQUEST.equalsIgnoreCase(scope)){
			Object obj = getRequestObj(name,property);
	...
	public String getItemsKeyName() {
		return itemsKeyName;
	}

	public void setItemsKeyName(String itemsKeyName) {
		this.itemsKeyName = itemsKeyName;
		
		
	}

	public String getItemskeyProperty() {
		return itemskeyProperty;
	}

	public void setItemskeyProperty(String itemskeyProperty) {
		this.itemskeyProperty = itemskeyProperty;
	}

	public String getMaxPageItemsKeyName() {
		return maxPageItemsKeyName;
	}

	public void setMaxPageItemsKeyName(String maxPageItemsKeyName) {
		this.maxPageItemsKeyName = maxPageItemsKeyName;
		
		
	}

	public String getMaxPageItemsKeyProperty() {
		return maxPageItemsKeyProperty;
	}

	public void setMaxPageItemsKeyProperty(String maxPageItemsKeyProperty) {
		this.maxPageItemsKeyProperty = maxPageItemsKeyProperty;
	}
	...
	private Object getRequestObj(String name,String property){
		...
		Object objReq = request.getAttribute(name);
		...
		try {
			...
			pagerObj = PropertyUtils.getNestedProperty(objReq,property);
			
			System.err.println("run nested ok.......");
			
			} catch (Exception ne) {
			ne.printStackTrace();
			System.err.println("run nested error.......");
			}
		}
	...
	private void test(){
		...
		if(StringUtils.isNotEmpty( itemsKeyName)){
			Object requestObjItems =
				(getRequestObj(itemsKeyName,itemskeyProperty));
			
			if(null == requestObjItems){
				return;
			}
		...
		if(StringUtils.isNotEmpty( maxPageItemsKeyName)){
			Object requestObjMaxPageItems =
				(getRequestObj(maxPageItemsKeyName, maxPageItemsKeyProperty));
			
			if(null == requestObjMaxPageItems){
				return;
			}
